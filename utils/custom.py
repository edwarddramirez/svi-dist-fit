...
def svi_loop_poisson(rng_key, num_steps, svi, x, pred, gp_rng_key = jax.random.PRNGKey(0)):
    '''
    Simple Loop for SVI optimization in 03_kl_poisson.ipynb notebook (for vanilla GP)
    Identical to above svi_loop function but replace xu and y with pred
    '''
    # svi update function
    def body_fn(i, svi_state):
        gp_rng_key = jax.random.split(svi_state.rng_key)[-1]
...
def svi_loop_poisson_sparse(rng_key, num_steps, svi, x, xu, pred, params, gp_rng_key = jax.random.PRNGKey(0)):
    '''
    Simple Loop for SVI optimization in 03_kl_poisson.ipynb notebook (for sparse GP)
    Identical to above svi_loop_poisson function but take two additional arguments
    xu and params 
    '''
    # svi update function
    def body_fn(i, svi_state):
        gp_rng_key = jax.random.split(svi_state.rng_key)[-1]
        svi_state, train_loss = svi.update(svi_state, x, xu, pred, params, gp_rng_key)
        return svi_state, train_loss
    
    # initial svi state
    svi_state = svi.init(rng_key, x, xu, pred, params, gp_rng_key)
